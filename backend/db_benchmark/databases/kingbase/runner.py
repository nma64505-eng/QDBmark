...
from db_benchmark.benchmark import _check_kingbase_connectivity, _run_kingbase_benchmark
...
def run(config: BenchmarkConfig, log_callback: Optional[LogCallback] = None, cancel_callback: Optional[CancelCallback] = None) -> BenchmarkSummary:
    return _run_kingbase_benchmark(config, log_callback=log_callback, cancel_callback=cancel_callback)
